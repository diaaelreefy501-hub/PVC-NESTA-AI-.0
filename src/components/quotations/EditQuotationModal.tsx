...
import { CompanyId, QuotationItem, Quotation } from "../../types";
import {
  FileSpreadsheet,
  Zap,
  Layers,
...
export const EditQuotationModal: React.FC<EditQuotationModalProps> = ({
  ...
  const { customers, companies, activeCompanyId, updateQuotation } = useApp();

  // Mode: "quick" for rapid total + description, "detailed" for itemized dimensions
  const [entryMode, setEntryMode] = useState<"quick" | "detailed">(quotation.isSummaryQuote ? "quick" : "detailed");

  // General deal info
  const [customerId, setCustomerId] = useState(quotation.customerId);
  const [companyId, setCompanyId] = useState<CompanyId>(quotation.companyId);
  const [notes, setNotes] = useState(quotation.notes || "");
  const [expiryDays, setExpiryDays] = useState(15);

  // Quick Mode inputs
  const [quickAmount, setQuickAmount] = useState<number | "">(quotation.totalAmount);
  const [quickDescription, setQuickDescription] = useState(
    quotation.summaryDescription || "توريد وتركيب شبابيك وأبواب UPVC شامل الزجاج والإكسسوارات"
  );
      ...
      <div className="bg-white rounded-3xl max-w-2xl w-full shadow-2xl border border-[#EAEAEA] overflow-hidden my-auto animate-in fade-in" dir="rtl">
        ...
        <div className="p-4 sm:p-5 bg-[#111111] text-white flex items-center justify-between border-b border-[#222222]">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-extrabold text-sm sm:text-base">تعديل عرض السعر</h3>
